...
| 2026-05-03 | **Send USDC to external address (basic wallet send)** | Wallet kita sekarang cuma punya 2 jalur: terima USDC + bayar QRIS. Gak ada send-out. User akan complain "saya mau pindahin USDC ke Indodax" atau "kirim ke teman". Standard Web3 wallet feature (Phantom, Solflare, Backpack semua punya). Implementasi: address validation (base58, 32-44 char, lamports check), amount input + max button, fee estimation, Privy biometric prompt, confirm modal, success state + Solana Explorer link, error handling (insufficient, invalid, network). **Hackathon decision (Day 11)**: skip — bukan differentiator, dilute pitch ke "another generic wallet", workaround tersedia via export private key di /settings + import ke Phantom. **V1.1**: ship dengan UI mirip /pay tapi destination bebas. | 4-6 jam | Defer V1.1 |
| 2026-05-03 | **Re-enable @handle (username) UI** | Migration `0008_user_handle.sql` + backend endpoints (`PATCH /users/handle`, `GET /users/by-handle/:handle`) sudah live di code. UI di /settings + /receive sengaja di-revert (commit Day 11) karena tanpa **send-out feature**, @handle gak bisa dipakai oleh wallet eksternal — Phantom dkk gak resolve handle Solana (Solana gak punya naming standard kayak ENS Ethereum). **V1.1 (paired dengan send-out)**: re-enable HandleCard di /settings + quick-share card di /receive. Tambah `/handle/:handle` page (RSC, public) di frontend untuk eksternal client visit `dollarkilat.xyz/sarah` → liat QR + address + send-via-our-app button (kalau install). **V1.2**: send-by-handle inside app (user → user via @handle resolution). | UI re-enable: 0.5 jam. Public profile page: 2-3 jam. Send-by-handle: 1 hari (depends on V1.1 send-out). | Defer V1.1 |
